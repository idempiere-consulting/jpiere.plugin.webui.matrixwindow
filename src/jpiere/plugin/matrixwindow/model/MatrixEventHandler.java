...
public class MatrixEventHandler extends AbstractEventHandler {
	...
	protected void doHandleEvent(Event event) {
		PO po = getPO(event);
		if(po.get_TableName().equals("M_InOutLine") && po.columnExists("LIT_M_Product_Category_ID") && po.columnExists("LineNetAmt") && po.is_ValueChanged("QtyEntered")) {
			if(event.getTopic().equals(IEventTopics.PO_AFTER_NEW) || event.getTopic().equals(IEventTopics.PO_AFTER_CHANGE)){
				...
				BigDecimal sumLineNetAmt = new Query(Env.getCtx(), "M_InOutLine", "ShipDate=? AND LIT_M_Product_Category_ID=? AND M_InOutLine_ID !=?", null)
						.setClient_ID()
						.setOnlyActiveRecords(true)
						.setParameters(shipDate, po.get_ValueAsInt("LIT_M_Product_Category_ID"), po.get_ValueAsInt("M_InOutLine_ID"))
						.sum("LineNetAmt");
